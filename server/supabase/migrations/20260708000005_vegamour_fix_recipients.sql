-- Fix: remove dynamic recipient lookup (returned full row including 7KB email_design, confusing Claude)
-- Replace with hardcoded recipients that are synced from auto_client_updates.email_recipients
-- The PATCH handler now keeps these in sync whenever UI recipients change

UPDATE team_cron_jobs
SET task = replace(
  task,
  $OLD$STEP 3 — Look up current recipients and send email:
...
C. Send the fully filled HTML email:
  ...
  $NEW$STEP 3 — Send email with the fully filled HTML:
CRITICAL: Pass the completed HTML DIRECTLY as the body. Do NOT wrap in markdown code fences. Body MUST start with <!DOCTYPE html>.
