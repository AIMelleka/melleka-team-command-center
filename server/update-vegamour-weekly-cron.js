...
STEP 5 — Save brief for PDF download (do this BEFORE sending the email):
Call save_vegamour_brief with:
- html: the exact HTML you built in Step 4 (all placeholders filled in)
- token: "weekly-{Sunday end date of THIS WEEK in YYYY-MM-DD}" (e.g. "weekly-2026-07-13")
The tool returns a download URL.

STEP 6 — Add a download button to the email HTML, then send:
Insert this row immediately before the footer row (the row with "Generated by Melleka AI Strategist"):
<tr><td style="padding:16px 40px;text-align:center;border-top:1px solid #e8e0f5;"><a href="[URL from Step 5]" style="display:inline-block;background:#2d1b4e;color:#fff;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;font-weight:600;letter-spacing:0.3px;">&#x2193; Download PDF</a></td></tr>
Then send ONE email with the updated HTML (including the button):
...
STEP 7 — After successful send, update dedup flag:
...
STEP 8 — Log completion to super_agent_task.
